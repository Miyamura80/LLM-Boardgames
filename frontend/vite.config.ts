...
// The frontend is an optional visualization layer over the `shbench serve`
// HTTP API. `vite` is invoked with this directory as its root (see the
// `dev`/`build` scripts in the repo-root package.json).
//
// In development, `/api` (and `/healthz`) are proxied to `shbench serve` so the
...
export default defineConfig({
	plugins: [react()],
	// Don't clear the terminal — keeps `shbench serve` logs visible alongside Vite.
	clearScreen: false,
	server: {
		port: 1420,
		strictPort: true,
