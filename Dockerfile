...
COPY crates ./crates
# `shbench new` embeds this template via include_str!; the (ungated) scaffold
# module needs it present at compile time, so it must be in the build context.
COPY templates ./templates
RUN cargo build --locked --release -p shbench
...
COPY --from=builder /app/target/release/shbench /usr/local/bin/shbench
...
COPY --from=builder /app/crates/config/global_config.yaml /etc/shbench/global_config.yaml
ENV APP_CONFIG_PATH=/etc/shbench/global_config.yaml
...
RUN useradd --system --no-create-home --uid 10001 shbench
USER shbench

ENTRYPOINT ["shbench", "serve"]
